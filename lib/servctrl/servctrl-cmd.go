...
func Execute(command, origin string) (string, *errco.MshLog) {
	...
	case !ServTerm.IsActive:
		return "", errco.NewLog("Execute", errco.TYPE_ERR, errco.LVL_2, errco.ERROR_TERMINAL_NOT_ACTIVE, "terminal not active")
	case servstats.Stats.Status != errco.SERVER_STATUS_ONLINE:
		return "", errco.NewLog("Execute", errco.TYPE_ERR, errco.LVL_2, errco.ERROR_SERVER_NOT_ONLINE, "server not online")
	case servstats.Stats.Suspended:
		return "", errco.NewLog("Execute", errco.TYPE_ERR, errco.LVL_2, errco.ERROR_SERVER_SUSPENDED, "server is suspended")
	}

	errco.Logln("Execute", errco.TYPE_INF, errco.LVL_2, errco.ERROR_NIL, "ms command: %s%s%s\t(origin: %s)", errco.COLOR_YELLOW, command, errco.COLOR_RESET, origin)

	// write to server terminal (\n indicates the enter key)
	_, err := ServTerm.inPipe.Write([]byte(command + "\n"))
	if err != nil {
		return "", errco.NewLog("Execute", errco.TYPE_ERR, errco.LVL_2, errco.ERROR_PIPE_INPUT_WRITE, err.Error())
	}
...
func TellRaw(reason, text, origin string) *errco.MshLog {
	switch {
	case !ServTerm.IsActive:
		return errco.NewLog("TellRaw", errco.TYPE_ERR, errco.LVL_2, errco.ERROR_TERMINAL_NOT_ACTIVE, "terminal not active")
	case servstats.Stats.Status != errco.SERVER_STATUS_ONLINE:
		return errco.NewLog("TellRaw", errco.TYPE_ERR, errco.LVL_2, errco.ERROR_SERVER_NOT_ONLINE, "server not online")
	case servstats.Stats.Suspended:
		return errco.NewLog("TellRaw", errco.TYPE_ERR, errco.LVL_2, errco.ERROR_SERVER_SUSPENDED, "server is suspended")
	}

	gameMessage, err := json.Marshal(&model.GameRawMessage{Text: "[MSH] " + reason + ": " + text, Color: "aqua", Bold: false})
	if err != nil {
		return errco.NewLog("TellRaw", errco.TYPE_ERR, errco.LVL_2, errco.ERROR_JSON_MARSHAL, err.Error())
	}

	gameMessage = append([]byte("tellraw @a "), gameMessage...)
	gameMessage = append(gameMessage, []byte("\n")...)

	errco.Logln("TellRaw", errco.TYPE_INF, errco.LVL_2, errco.ERROR_NIL, "ms tellraw: %s%s%s\t(origin: %s)", errco.COLOR_YELLOW, string(gameMessage), errco.COLOR_RESET, origin)

	// write to server terminal (\n indicates the enter key)
	_, err = ServTerm.inPipe.Write(gameMessage)
	if err != nil {
		return errco.NewLog("TellRaw", errco.TYPE_ERR, errco.LVL_2, errco.ERROR_PIPE_INPUT_WRITE, err.Error())
	}
...
func termStart(dir, command string) *errco.MshLog {
	if ServTerm.IsActive {
		errco.Logln("termStart", errco.TYPE_WAR, errco.LVL_3, errco.ERROR_SERVER_IS_WARM, "minecraft server terminal already active")
		return nil
	}

	logMsh := termLoad(dir, command)
	if logMsh != nil {
		return logMsh.AddTrace("termStart")
	}
	...
	if err != nil {
		return errco.NewLog("termStart", errco.TYPE_ERR, errco.LVL_3, errco.ERROR_TERMINAL_START, err.Error())
	}
...
func termLoad(dir, command string) *errco.MshLog {
	...
	if err != nil {
		return errco.NewLog("termLoad", errco.TYPE_ERR, errco.LVL_3, errco.ERROR_PIPE_LOAD, "StdoutPipe load: "+err.Error())
	}
	ServTerm.errPipe, err = ServTerm.cmd.StderrPipe()
	if err != nil {
		return errco.NewLog("termLoad", errco.TYPE_ERR, errco.LVL_3, errco.ERROR_PIPE_LOAD, "StderrPipe load: "+err.Error())
	}
	ServTerm.inPipe, err = ServTerm.cmd.StdinPipe()
	if err != nil {
		return errco.NewLog("termLoad", errco.TYPE_ERR, errco.LVL_3, errco.ERROR_PIPE_LOAD, "StdinPipe load: "+err.Error())
	}
...
func printerOutErr() {
	...
	go func() {
		...
		for scanner.Scan() {
			...
			errco.Logln("printerOutErr", errco.TYPE_SER, errco.LVL_2, errco.ERROR_NIL, line)
			...
			case errco.SERVER_STATUS_STARTING:
				...
				if strings.Contains(line, "INFO") && strings.Contains(line, ": Done (") {
					...
					errco.Logln("printerOutErr", errco.TYPE_INF, errco.LVL_1, errco.ERROR_NIL, "MINECRAFT SERVER IS ONLINE!")
			...
			case errco.SERVER_STATUS_ONLINE:
				...
				if !strings.Contains(line, ": ") {
					errco.Logln("printerOutErr", errco.TYPE_WAR, errco.LVL_2, errco.ERROR_SERVER_UNEXP_OUTPUT, "line does not adhere to expected log format")
					continue
				}
				...
				if strings.Contains(lineHeader, "INFO") {
					...
					case strings.HasPrefix(lineContent, "<") || strings.HasPrefix(lineContent, "["):
						...
						errco.Logln("printerOutErr", errco.TYPE_INF, errco.LVL_2, errco.ERROR_NIL, "a chat message was sent")
					...
					case strings.Contains(lineContent, "UUID of player"):
						...
						errco.Logln("printerOutErr", errco.TYPE_INF, errco.LVL_2, errco.ERROR_NIL, "A PLAYER JOINED THE SERVER! - %d players online", servstats.Stats.PlayerCount)
					...
					case strings.Contains(lineContent, "lost connection"):
						servstats.Stats.PlayerCount--
						errco.Logln("printerOutErr", errco.TYPE_INF, errco.LVL_2, errco.ERROR_NIL, "A PLAYER LEFT THE SERVER! - %d players online", servstats.Stats.PlayerCount)
						FreezeMSRequest()
					...
					case strings.Contains(lineContent, "Stopping") && strings.Contains(lineContent, "server"):
						...
						errco.Logln("printerOutErr", errco.TYPE_INF, errco.LVL_1, errco.ERROR_NIL, "MINECRAFT SERVER IS STOPPING!")
					}
				}
			}
		}
...
func waitForExit() {
	...
	errco.Logln("waitForExit", errco.TYPE_INF, errco.LVL_1, errco.ERROR_NIL, "MINECRAFT SERVER IS STARTING!")

	ServTerm.IsActive = true
	errco.Logln("waitForExit", errco.TYPE_INF, errco.LVL_3, errco.ERROR_NIL, "waitForExit: terminal started")
	...
	errco.Logln("waitForExit", errco.TYPE_INF, errco.LVL_3, errco.ERROR_NIL, "waitForExit: terminal exited")

	servstats.Stats.Status = errco.SERVER_STATUS_OFFLINE
	servstats.Stats.Suspended = false

	errco.Logln("waitForExit", errco.TYPE_INF, errco.LVL_1, errco.ERROR_NIL, "MINECRAFT SERVER IS OFFLINE!")
}
