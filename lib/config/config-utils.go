...
func (c *Configuration) InWhitelist(params ...string) *errco.MshLog {
	...
	if len(c.Msh.Whitelist) == 0 {
		errco.Logln(errco.Orig(), errco.TYPE_INF, errco.LVL_3, errco.ERROR_NIL, "whitelist not enabled")
		return nil
	}

	errco.Logln(errco.Orig(), errco.TYPE_INF, errco.LVL_3, errco.ERROR_NIL, "checking whitelist for: %s", strings.Join(params, ", "))

	// check if playerName or clientAddress are in whitelist
	for _, p := range params {
		if utility.SliceContain(p, c.Msh.Whitelist) {
			errco.Logln(errco.Orig(), errco.TYPE_INF, errco.LVL_3, errco.ERROR_NIL, "playerName or clientAddress is whitelisted!")
			return nil
		}
	}

	// playerName or clientAddress not found in whitelist
	return errco.NewLog(errco.Orig(), errco.TYPE_ERR, errco.LVL_1, errco.ERROR_PLAYER_NOT_IN_WHITELIST, "playerName or clientAddress is not whitelisted")
}
...
func (c *Configuration) loadIcon() *errco.MshLog {
	...
	for _, uip := range userIconPaths {
		...
		f, err := os.Open(uip)
		if err != nil {
			errco.Logln(errco.Orig(), errco.TYPE_ERR, errco.LVL_3, errco.ERROR_ICON_LOAD, err.Error())
			continue
		}
		defer f.Close()
		...
		if img, err = png.Decode(bytes.NewReader(fdata)); err == nil {
			...
			errco.Logln(errco.Orig(), errco.TYPE_ERR, errco.LVL_3, errco.ERROR_ICON_LOAD, "data format invalid: %s (%s)", uip, err.Error())
			continue
		}

		// scale image to 64x64
		scaImg, d := utility.ScaleImg(img, image.Rect(0, 0, 64, 64))
		errco.Logln(errco.Orig(), errco.TYPE_INF, errco.LVL_3, errco.ERROR_NIL, "scaled %s to 64x64. (%v ms)", uip, d.Milliseconds())
...
func (c *Configuration) loadIpPorts() *errco.MshLog {
	...
	if err != nil {
		return errco.NewLog(errco.Orig(), errco.TYPE_ERR, errco.LVL_3, errco.ERROR_CONVERSION, err.Error())
	}

	if TargetPort == c.Msh.ListenPort {
		return errco.NewLog(errco.Orig(), errco.TYPE_ERR, errco.LVL_1, errco.ERROR_CONFIG_LOAD, "TargetPort and ListenPort appear to be the same, please change one of them")
	}
...
func (c *Configuration) getVersionInfo() (string, int, *errco.MshLog) {
	...
	if err != nil {
		return "", 0, errco.NewLog(errco.Orig(), errco.TYPE_ERR, errco.LVL_3, errco.ERROR_VERSION_LOAD, err.Error())
	}
	defer reader.Close()
	...
	return "", 0, errco.NewLog(errco.Orig(), errco.TYPE_ERR, errco.LVL_3, errco.ERROR_VERSION_LOAD, "minecraft server version and protocol could not be extracted from version.json")
}
