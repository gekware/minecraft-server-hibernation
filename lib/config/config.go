...
func LoadConfig() *errco.MshLog {
	...
	errco.Logln(errco.Orig(), errco.TYPE_INF, errco.LVL_3, errco.ERROR_NIL, "checking OS support...")

	// check if OS is supported.
	logMsh := opsys.OsSupported()
	if logMsh != nil {
		return logMsh.AddTrace("LoadConfig")
	}

	// ---------------- load config ---------------- //

	errco.Logln(errco.Orig(), errco.TYPE_INF, errco.LVL_3, errco.ERROR_NIL, "loading config...")
...
func (c *Configuration) Save() *errco.MshLog {
	...
	if err != nil {
		return errco.NewLog(errco.Orig(), errco.TYPE_ERR, errco.LVL_3, errco.ERROR_CONFIG_SAVE, "could not marshal from config file")
	}

	// escape unicode characters ("\u003c" to "<" and "\u003e" to ">")
	configData, logMsh := utility.UnicodeEscape(configData)
	if logMsh != nil {
		errco.Log(logMsh.AddTrace("Save"))
	}

	// write to config file
	err = os.WriteFile(configFileName, configData, 0644)
	if err != nil {
		return errco.NewLog(errco.Orig(), errco.TYPE_ERR, errco.LVL_3, errco.ERROR_CONFIG_SAVE, "could not write to config file")
	}

	errco.Logln(errco.Orig(), errco.TYPE_INF, errco.LVL_3, errco.ERROR_NIL, "saved default config to config file")
...
func (c *Configuration) loadDefault() *errco.MshLog {
	...
	if err != nil {
		return errco.NewLog(errco.Orig(), errco.TYPE_ERR, errco.LVL_1, errco.ERROR_CONFIG_LOAD, err.Error())
	}

	// read config file
	configFilePath := filepath.Join(cwdPath, configFileName)
	errco.Logln(errco.Orig(), errco.TYPE_INF, errco.LVL_3, errco.ERROR_NIL, "reading config file: \"%s\"", configFilePath)
	...
	if c.Configuration.Msh.ID != mi {
		errco.Logln(errco.Orig(), errco.TYPE_WAR, errco.LVL_3, errco.ERROR_CONFIG_LOAD, "config msh id different from instance msh id, applying correction...")
	...
	if logMsh != nil {
		...
		errco.Log(logMsh.AddTrace("loadDefault"))
...
func (c *Configuration) loadRuntime(confdef *Configuration) *errco.MshLog {
	...
	*c = *confdef
	...
	errco.Logln(errco.Orig(), errco.TYPE_INF, errco.LVL_0, errco.ERROR_NIL, "setting log level to: %d", c.Msh.Debug)
	...
	if _, err := os.Stat(serverFileFolderPath); os.IsNotExist(err) {
		...
		logMsh := errco.NewLog(errco.Orig(), errco.TYPE_ERR, errco.LVL_1, errco.ERROR_MINECRAFT_SERVER, "specified minecraft server folder/file does not exist: %s", serverFileFolderPath)
		...
		case err != nil:
			...
			errco.Logln(errco.Orig(), errco.TYPE_WAR, errco.LVL_1, errco.ERROR_CONFIG_CHECK, "could not read eula.txt file: %s", eulaFilePath)

			// start server to generate eula.txt (and server.properties)
			errco.Logln(errco.Orig(), errco.TYPE_INF, errco.LVL_3, errco.ERROR_NIL, "starting minecraft server to generate eula.txt file...")
			...
			if err != nil {
				logMsh := errco.NewLog(errco.Orig(), errco.TYPE_ERR, errco.LVL_1, errco.ERROR_MINECRAFT_SERVER, "couldn't start minecraft server to generate eula.txt (%s)", err.Error())
				servstats.Stats.SetMajorError(logMsh)
				errco.Log(logMsh)
			}
			fallthrough
		...
		case !strings.Contains(strings.ReplaceAll(strings.ToLower(string(eulaData)), " ", ""), "eula=true"):
			...
			logMsh := errco.NewLog(errco.Orig(), errco.TYPE_ERR, errco.LVL_1, errco.ERROR_MINECRAFT_SERVER, "please accept minecraft server eula.txt: %s", eulaFilePath)
			servstats.Stats.SetMajorError(logMsh)
			errco.Log(logMsh)
		...
		default:
			...
			errco.Logln(errco.Orig(), errco.TYPE_INF, errco.LVL_1, errco.ERROR_NIL, "eula.txt exist and is set to true")
		}
	}
	...
	if err != nil {
		logMsh := errco.NewLog(errco.Orig(), errco.TYPE_ERR, errco.LVL_1, errco.ERROR_MINECRAFT_SERVER, "java not installed")
		servstats.Stats.SetMajorError(logMsh)
		errco.Log(logMsh)
	} else if out, err := exec.Command("java", "--version").Output(); err != nil {
		// non blocking error
		errco.Logln(errco.Orig(), errco.TYPE_WAR, errco.LVL_1, errco.ERROR_CONFIG_CHECK, "could not execute 'java -version' command")
		Javav = "unknown"
	} else {
	...
	if logMsh != nil {
		logMsh.AddTrace("loadRuntime")
		servstats.Stats.SetMajorError(logMsh)
		errco.Log(logMsh)
	} else {
		errco.Logln(errco.Orig(), errco.TYPE_INF, errco.LVL_3, errco.ERROR_NIL, "msh proxy setup: %s:%d --> %s:%d", ListenHost, ListenPort, TargetHost, TargetPort)
	}
	...
	if logMsh != nil {
		...
		errco.Log(logMsh.AddTrace("loadRuntime"))
	}
